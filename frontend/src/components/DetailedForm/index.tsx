...
const DetailedForm = () => {
  ...
  const roofConfiguration = JSON.parse(localStorage.getItem("roofConfigurator")!);

  const {
    handleSubmit,
    formState: { errors },
    register,
  } = useForm<DetailedConfiguratorState>();
    ...
    <>
      <Card full={true} title="Szczegółowe informacje">
        <form className="grid grid-cols-3 gap-6" onSubmit={handleSubmit(onSubmit)}>
          <Input
            defaultValue={(parseInt(roofConfiguration.solar_amount) * 0.35 * 5000).toFixed(2)}
            unit="PLN"
          ...
          <Input
            defaultValue={((data?.features[0]?.properties?.solar_panels?.flat?.solar_output?.partly_cloudy?.estimated_production_per_hour / 1000) * parseInt(roofConfiguration.solar_amount)).toFixed(
              2
            )}
            unit="kWh"
