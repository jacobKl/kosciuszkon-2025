...
const Input = ({ label, register, name, helper, error }: { label: string; register: UseFormRegisterReturn; name: string; helper?: string; error: any }) => {
  return (
    <div className="flex flex-col relative w-full">
      ...
      <label
        htmlFor={name}
        className="absolute font-thin left-0 -top-3.5 text-gray-600 text-sm transition-all peer-placeholder-shown:text-base peer-placeholder-shown:text-gray-400 peer-placeholder-shown:top-3 peer-focus:-top-3.5 peer-focus:text-gray-600 peer-focus:text-sm"
      >
        {label}
      </label>
